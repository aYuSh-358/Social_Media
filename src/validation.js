...
exports.validator = (method) => {
    switch (method) {
        ...
        // Create Register User Validation 
        case 'registerUser':
            {
                return [
                    ...
                    //         .normalizeEmail().isLength({ max: 30 }).withMessage('Email should not exceed 30 characters'),


                    //     body('userPassword').isLength({ min: 8 }).withMessage('Password must be at least 8 characters long')
                    ...
                    //         .isISO8601().withMessage('DOB must be a valid date (YYYY-MM-DD format)'),

                    //     body('userMobileNo')
                    //         .notEmpty().withMessage('Mobile Number is required')
                    //         .isLength({ min: 10, max: 10 }).withMessage('Mobile Number must be 10 digits')
                    //         .matches(/^[0-9]+$/).withMessage('Mobile Number must contain only numerical values')
        ...
        // Update  Register User Validation 
        case 'updateRegisterUser':
            {
                return [

                    // body('userName').optional().notEmpty().withMessage('Name is required'),

                    // body('userEmail').optional().notEmpty().isEmail().withMessage('Invalid email address')
                    //     .normalizeEmail().isLength({ max: 30 }).withMessage('Email should not exceed 30 characters'),

                    // body('userPassword').optional().isLength({ min: 8 }).withMessage('Password must be at least 8 characters long')
                    //     .matches(/[a-z]/).withMessage('Password must contain at least one lowercase')
                    //     .matches(/[A-Z]/).withMessage('Password must contain at least one uppercase')
                    //     .matches(/[0-9]/).withMessage('Password must contain at least one number')
                    //     .matches(/[@$!%*?&]/).withMessage('Password must contain at least one special character (@,$,!,%,*,?,&)'),

                    // body('userDOB').optional()
                    //     .notEmpty().withMessage('Date of Birth is required')
                    //     .isISO8601().withMessage('DOB must be a valid date (YYYY-MM-DD format)'),
